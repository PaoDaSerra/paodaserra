...
function displayCartItems() {
  ...
  itemsCarrinho.forEach(item => {
    ...
    <div class="quantidade">
        ...
        <button class="lixo" type="button" onclick="deletarItem()">
  ...
  const lixo = document.querySelectorAll(".lixo");
  lixo.forEach((item, indice) => {
...
function sendMessage() {
  console.log(itemsCarrinho.length != 0)
  if (itemsCarrinho.length != 0) {
    ...
    link += `%0A*Total:*                                    %20R$${totalPrice.toFixed(2)}`
    
    window.open(link, '_blank');
  } else {
    window.alert("Seu carrinho está vazio. Adicione itens para finalizar o pedido!")
  }
}